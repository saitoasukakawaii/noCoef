...
void Tube :: printCxt (FILE *fd, double t, int offset)
{
  for (int i=0; i<N; i++)
  {
    fprintf (fd, "%13.10f %13.10f %15.10f\n",
             t*Lr3/q, (i+offset)*h*Lr, c(i, Anew[i])*q/Lr2);
  }
}

void Tube :: printUxt (FILE *fd, double t, int offset)
{
  for (int i=0; i<N; i++)
  {
    fprintf (fd, "%13.10f %13.10f %15.10f\n",
...
void Tube :: bound_bif_left (double theta, double gamma)
{
  double PN;
  int j = 1;
  int ok = false;
  ...
  xb[ 0] =  Qh[0];                              //Initial guess for Q1_xb n+1
  xb[ 1] = (Qold[1] + Qold[0])/2;               //Initial guess for Q1_xb^n+0.5
  xb[ 2] =  Qold[0];                            //Initial guess for Q1_xb+0.5 n+0.5
  xb[ 3] =  LP->Qh[LPN-1];                      //Initial guess for Q2_xb n+1
  ...
  xb[11] =  Aold[0];                            //Initial guess for A1_xb+0.5 n+0.5
  xb[12] =  LP->Ah[LPN-1];                      //Initial guess for A2_xb n+1
  xb[13] = (LP->Aold[LPN] + LP->Aold[LPN-1])/2; //Initial guess for A2_xb n+0.5
  xb[14] =  LP->Aold[LPN];                      //Initial guess for A2_xb+0.5 n+0.5
  xb[15] =  RP->Ah[RPN-1];                      //Initial guess for A3_xb n+1
  xb[16] = (RP->Aold[RPN] + RP->Aold[RPN-1])/2; //Initial guess for A3_xb n+0.5
  xb[17] =  RP->Aold[RPN];                      //Initial guess for A3_xb+0.5 n+0.5

  double k7nh  = K_loss_LP/2; //32*mu/(2*LD->rtop*rho*q);
  ...
  while (j <= ntrial && ok==false) // Find the zero
  {
    ...
    // The residuals.
    fvec[0]  = g1  - xb[0] +
                theta*(sq(xb[2])/xb[11] + Bh(-1,xb[11])) +
	              gamma*(F(xb[2],xb[11])+dBdx1h(-1,xb[11]));
    fvec[1]  = g2  - xb[3] -
	              theta*(sq(xb[5])/xb[14] + LP->Bh(LPN,xb[14])) +
	              gamma*(F(xb[5],xb[14])  + LP->dBdx1h(LPN,xb[14]));
    fvec[2]  = g2a - xb[6] -
	              theta*(sq(xb[8])/xb[17] + RP->Bh(RPN,xb[17])) +
	              gamma*(F(xb[8],xb[17])  + RP->dBdx1h(RPN,xb[17]));
...
#ifdef TOTAL_PRESSURE
    PN    = P(0,xb[10]);
    double sq211 = sq(xb[1]/xb[10]);
    if (xb[1] > 0)
    {
      fvec[14] =  - PN + LP->P(LPN,xb[13]) - k7nh*sq211 - 0.5*(sq211 - sq(xb[4]/xb[13]));
      fvec[15] =  - PN + RP->P(RPN,xb[16]) - k7anh*sq211 - 0.5*(sq211 - sq(xb[7]/xb[16]));
    } else
    {
      fvec[14] =  - PN + LP->P(LPN,xb[13]) + k7nh*sq211 - 0.5*(sq211 - sq(xb[4]/xb[13]));
      fvec[15] =  - PN + RP->P(RPN,xb[16]) + k7anh*sq211 - 0.5*(sq211 - sq(xb[7]/xb[16]));
    ...
    if (xb[0] > 0)
    {
      fvec[16] = - PN + LP->P(LPN,xb[12]) - k7n*sq110 - 0.5*(sq110 - sq(xb[3]/xb[12]));
      fvec[17] = - PN + RP->P(RPN,xb[15]) - k7an*sq110 - 0.5*(sq110 - sq(xb[6]/xb[15]));
    } else
    {
      fvec[16] = - PN + LP->P(LPN,xb[12]) + k7n*sq110 - 0.5*(sq110 - sq(xb[3]/xb[12]));
      fvec[17] = - PN + RP->P(RPN,xb[15]) + k7an*sq110 - 0.5*(sq110 - sq(xb[6]/xb[15]));
    ...
    if (xb[1] > 0)
    {
      fvec[14] =  - PN + LP->P(LPN,xb[13]) - k7nh*sq211;
      fvec[15] =  - PN + RP->P(RPN,xb[16]) - k7anh*sq211;
    } else
    {
      fvec[14] =  - PN + LP->P(LPN,xb[13]) + k7nh*sq211;
      fvec[15] =  - PN + RP->P(RPN,xb[16]) + k7anh*sq211;
    ...
    if (xb[0] > 0)
    {
      fvec[16] = - PN + LP->P(LPN,xb[12]) - k7n*sq110;
      fvec[17] = - PN + RP->P(RPN,xb[15]) - k7an*sq110;
    } else
    {
      fvec[16] = - PN + LP->P(LPN,xb[12]) + k7n*sq110;
      fvec[17] = - PN + RP->P(RPN,xb[15]) + k7an*sq110;
    ...
    // flow equation
    fjac[ 0][ 0] = -1.0;
    fjac[ 0][ 2] = 2*theta*xb[2]/xb[11] + gamma*dFdQ(xb[11]);  		// xi_1
    fjac[ 0][11] = theta*( -sq(xb[2]/xb[11]) + dBdAh(-1,xb[11])) + 		// xi_2
                   gamma*(dFdA(xb[2],xb[11]) + d2BdAdxh(-1,xb[11]));          
    fjac[ 1][ 3] = -1.0;
    fjac[ 1][ 5] = -2*theta*xb[5]/xb[14] + gamma*dFdQ(xb[14]);		// xi_3
    fjac[ 1][14] = theta*(  sq(xb[5]/xb[14]) - LP->dBdAh(LPN,xb[14])) + 	// xi_4
                   gamma*(dFdA(xb[5],xb[14]) + LP->d2BdAdxh(LPN,xb[14]));
    fjac[ 2][ 6] = -1.0;
    fjac[ 2][ 8] = -2*theta*xb[8]/xb[17] + gamma*dFdQ(xb[17]);  	// xi_5
    fjac[ 2][17] = theta*(  sq(xb[8]/xb[17]) - RP->dBdAh(RPN,xb[17])) + 	// xi_6
                   gamma*(dFdA(xb[8],xb[17]) + RP->d2BdAdxh(RPN,xb[17]));
    // area equation
    fjac[ 3][ 2] =  theta;
    fjac[ 3][ 9] = -1.0;
    fjac[ 4][ 5] = -theta;
    ...
    if (xb[1] > 0)
    {
      ...
      fjac[14][10] = - dPdA(0,xb[10])
		            + sq(xb[1])/cu(xb[10])*(2*k7nh+1);
      fjac[15][10] = - dPdA(0,xb[10])
    ...
    fjac[14][13] = LP->dPdA(LPN,xb[13]) - sq(xb[4])/cu(xb[13]);
    fjac[15][16] = RP->dPdA(RPN,xb[16]) - sq(xb[7])/cu(xb[16]);
    if (xb[0] > 0)
    {
      fjac[16][0] = xb[0]/sq(xb[9])*(-2*k7n-1);
      fjac[17][0] = xb[0]/sq(xb[9])*(-2*k7an-1);
      fjac[16][9] = - dPdA(0,xb[9])
		              + sq(xb[0])/cu(xb[9])*(2*k7n+1);
      fjac[17][9] = - dPdA(0,xb[9])
    ...
    fjac[16][12] = LP->dPdA(0,xb[12]) - sq(xb[3])/cu(xb[12]);
    fjac[17][15] = RP->dPdA(0,xb[15]) - sq(xb[6])/cu(xb[15]);
#else 
    if (xb[1] > 0)
    {
      ...
      fjac[14][10] = - dPdA(0,xb[10])
		            + sq(xb[1])/cu(xb[10])*(2*k7nh);
      fjac[15][10] = - dPdA(0,xb[10])
	              + sq(xb[1])/cu(xb[10])*(2*k7anh);
    } else
    {
      fjac[14][1] = xb[1]/sq(xb[10])*(2*k7nh);
      fjac[15][1] = xb[1]/sq(xb[10])*(2*k7anh);
    ...
    fjac[14][13] = LP->dPdA(LPN,xb[13]);
    fjac[15][16] = RP->dPdA(RPN,xb[16]);
    if (xb[0] > 0)
    {
      fjac[16][0] = xb[0]/sq(xb[9])*(-2*k7n);
      fjac[17][0] = xb[0]/sq(xb[9])*(-2*k7an);
      fjac[16][9] = - dPdA(0,xb[9])
		              + sq(xb[0])/cu(xb[9])*(2*k7n);
      fjac[17][9] = - dPdA(0,xb[9])
		              + sq(xb[0])/cu(xb[9])*(2*k7an);
    } else
    {
      fjac[16][0] = xb[0]/sq(xb[9])*(2*k7n);
      fjac[17][0] = xb[0]/sq(xb[9])*(2*k7an);
    ...
    fjac[16][12] = LP->dPdA(LPN,xb[12]);
    fjac[17][15] = RP->dPdA(RPN,xb[15]);
#endif
...
void Tube :: bound_bif_right (double theta, double gamma)
{
  double PN;
  int j = 1;
  int ok = false;
  ...
  double k7nh  = K_loss_LD/2; //32*mu/(2*LD->rtop*rho*q);
  double k7n   = K_loss_LD/2; //32*mu/(2*RD->rtop*rho*q);
  double k7anh = K_loss_RD/2; //32*mu/(2*LD->rtop*rho*q);
  double k7an  = K_loss_RD/2; //32*mu/(2*RD->rtop*rho*q);
  ...
  while (j <= ntrial && ok==false) // Find the zero
  {
    ...
    if (xb[1] > 0)
    {
      fvec[14] =  - PN + LD->P(0,xb[13]) + k7nh*sq211 - 0.5*(sq211 - sq(xb[4]/xb[13]));
      fvec[15] =  - PN + RD->P(0,xb[16]) + k7anh*sq211 - 0.5*(sq211 - sq(xb[7]/xb[16]));
    } else
    {
      fvec[14] =  - PN + LD->P(0,xb[13]) - k7nh*sq211 - 0.5*(sq211 - sq(xb[4]/xb[13]));
      fvec[15] =  - PN + RD->P(0,xb[16]) - k7anh*sq211 - 0.5*(sq211 - sq(xb[7]/xb[16]));
    ...
    if (xb[0] > 0)
    {
      fvec[16] = - PN + LD->P(0,xb[12]) + k7n*sq110 - 0.5*(sq110 - sq(xb[3]/xb[12]));
      fvec[17] = - PN + RD->P(0,xb[15]) + k7an*sq110 - 0.5*(sq110 - sq(xb[6]/xb[15]));
    } else
    {
      fvec[16] = - PN + LD->P(0,xb[12]) - k7n*sq110 - 0.5*(sq110 - sq(xb[3]/xb[12]));
      fvec[17] = - PN + RD->P(0,xb[15]) - k7an*sq110 - 0.5*(sq110 - sq(xb[6]/xb[15]));
